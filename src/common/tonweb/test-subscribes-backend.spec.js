...
class BlocksStorageImpl {
  ...
  async isSubscription(address) {
    address = Address.parse(address).toString(false);
  ...
  async replaceSubscription(address, data) {
    data.address = Address.parse(address).toString(false);
...
async function init() {
  ...
  const processTransaction = async (address, tx) => {
    ...
    const subscription2 = new TonWeb.SubscriptionContract(tonweb.provider, {
      workChain: 0,
      wallet: Address.parse(subscriptionData.wallet),
      beneficiary: Address.parse(subscriptionData.beneficiary),
    ...
    if (Address.parse(subscriptionAddress).toString(false) !== validSubscriptionAddress.toString(false)) return false;
