...
import {hexToBytes, bytesToBase64} from "../../utils";


class SubscriptionContract extends Contract {
    /**
     * @param provider
     * @param options   {{wc: number, wallet: Address, beneficiary: Address, amount: BigNumber, period: number, timeout: number, startAt: number, subscriptionId: number, address?: Address | string}}
     */
    ...
    createBody() {
        const body = new Cell();
        body.bits.writeUint(new BigNumber(0x706c7567, 16).add(new BigNumber(0x80000000, 16)), 32); // op
        return body;
    }
