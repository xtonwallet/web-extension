...
import { loadOutAction, loadOutList, storeOutAction, storeOutList } from "./OutList";
import { SendMode } from "./SendMode";
import { storeMessageRelaxed } from "./MessageRelaxed";
...
describe('Out List', () => {
  ...
  it('Should deserialize sendMsg action', () => {
    ...
    const action = beginCell()
      ...
      .storeRef(beginCell().store(storeMessageRelaxed(mockMessageRelaxed1)).endCell())
      .endCell();
    const actual = loadOutAction(action.beginParse());
    const expected = {
      type: 'sendMsg',
      mode,
      outMsg: mockMessageRelaxed1
