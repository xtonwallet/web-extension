...
class BitString {
    ...
    writeUint(number, bitLength) {
        ...
        if (
            ...
            if (number.eq(new BigNumber(0))) return;
            throw Error("bitLength is too small for number, got number=" + number.toString() + ",bitLength=" + bitLength);
        }
        const s = number.toString(2).padStart(bitLength, "0");;
    ...
    writeInt(number, bitLength) {
        number = new BigNumber(number);
        if (bitLength == 1) {
            if (number.eq(new BigNumber(-1))) {
                this.writeBit(true);
                return;
            }
            if (number.eq(new BigNumber(0))) {
                this.writeBit(false);
                return;
            }
            ...
            if (number.isNegative()) {
                ...
                const nb = b.pow(new BigNumber(bitLength - 1));
    ...
    /**
     * @param amount  {number | BigNumber} in nanograms
     */
    writeGrams(amount) {
    ...
    /**
     * @param amount  {number | BigNumber} in nanotons
     */
    writeCoins(amount) {
